...
ov5640_reg_cfg_t ov5640_init_cfg[207] = {
    ...
    {0x3017, 0xFF},
    {0x3018, 0xFF},
    {OV5640_SC_PLL_CONTRL0, 0x1A},       //0x3034[3:0]，如果是8，则是2分频，如果是A则是2.5分频，如果是其他则为1分频
//    {OV5640_SC_PLL_CONTRL1, 0x41},
//    {OV5640_SC_PLL_CONTRL2, 0x30},
    {OV5640_SC_PLL_CONTRL3, 0x13},  //3037[3:0]分频  [4]?2:1分频
//    {OV5640_SC_PLL_CONTRL0, 0x1A},
//    {OV5640_SC_PLL_CONTRL3, 0x13},
    {0x3108, 0x01},                   //0x3108[5:4],00:1分频；01:2分频；10:4分频；11:8分频
    {0x3630, 0x36},
    {0x3631, 0x0E},
    {0x3632, 0xE2},
    {0x3633, 0x12},
...
ov5640_reg_cfg_t ov5640_rgb565_cfg[45] = {
    {0x4300, 0x6F},
    {0x501F, 0x01},             
	/*0x11+0x64=80mhz    pclkh7max=80mhz*/
    {OV5640_SC_PLL_CONTRL1, 0x11},      // 0x3035[7:4] 分频   如果是mipi2 lane,则分频系数是0x3035[3:0],如果是DVP 接口则分频系数为2*0x3035[3:0]
    {OV5640_SC_PLL_CONTRL2, 0x64},      //3036[6:0] 倍频
    {0x3C07, 0x07},
    {0x3820, 0x46},
    {0x3821, 0x00},
    {0x3814, 0x31},
